...
ctrHref = trimTrailingSlash('https://acadooghostwriter.com/'.trim());
ctrHref2 = trimTrailingSlash('https://acadooghostwriter.com/'.trim());
eInDoc = function(e) {
...
window.onload = function () {
    ...
    for (i = 0; i < l; i++) {
       ...
       if (trimTrailingSlash(el.href) === ctrHref || trimTrailingSlash(el.getAttribute('href')) === ctrHref || trimTrailingSlash(el.href) === ctrHref2 || trimTrailingSlash(el.getAttribute('href')) === ctrHref2) {
        ...
        if(linktext != 'AcadooGhostwriter.com') { linkfound = false; }
        break;
      }
    }
  ...
  linkToHide.innerHTML = '';
    ...
    if (linkfound == false) {
            ...
            div.id = 'error_';
            div.innerHTML = '<a href="">Counter Error: Do not change the code. Click here to show the correct code!</a>';
