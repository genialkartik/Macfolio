...
router.route('/file')
  .post((req, res, files) => {
    ...
    var customFiletype = req.files.filetoupload.mimetype.toString()
    var fileName = req.body.filenametoupload+'.'+customFiletype.substring(customFiletype.indexOf('/')+1, customFiletype.length)
    ...
    req.files.filetoupload.mv(staticPath + fileName.replace(/\s/g, ''), err => {
      if (err) {
        console.error(err);
